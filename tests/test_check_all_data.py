...
class TestCheckAllDataScript(unittest.TestCase):
    ...
    def test_check_all_data_prints_content_fix_hints_for_empty_davis(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            data_dir = tmp_path / "davis"
            data_dir.mkdir(parents=True, exist_ok=True)

            # Files exist but are empty, so this should trigger content-fix guidance.
            ...
            cfg = {
                "name": "C2DTI_DAVIS_ALL_CHECK_EMPTY",
                "protocol": "P1",
                "output": {"base_dir": str(tmp_path / "outputs")},
                "dataset": {
                    "name": "DAVIS",
            ...
            self.assertEqual(completed.returncode, 1)
            self.assertIn("Fix data content", completed.stdout)
            self.assertIn("Add at least one non-empty line", completed.stdout)
            self.assertIn("Fill", completed.stdout)