...
def run_once(config_path: str) -> int:
    ...
    summary_payload = {
        ...
        "notes": "Initial run contract step (no model training yet)."
    }
    ...
    if dataset_cfg:
        ...
        if bool(dataset.metadata.get("is_placeholder", False)) and not allow_placeholder:
            print("[ERROR] Dataset fallback path was used but strict dataset mode is enabled")
            print(f"[ERROR] Please provide source dataset files at: {dataset_cfg.get('path')}")
            return 3
        ...
        # evaluate on all known pairs (useful for strict checks and dry-runs).
        ...
        summary_payload["notes"] = "DTI pipeline completed with dataset loading, split, prediction, evaluation, and optional causal analysis."
        ...
        if test_mask is not None:
            ...
            # No split: evaluate on all known pairs (strict-check / fallback mode)
